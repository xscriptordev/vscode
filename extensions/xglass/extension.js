...
function activate(context) {
    if (process.platform == 'win32') {
        ...
        function setAlpha(alpha) {
            ...
            ps.invoke().then(res => {
                ...
                console.error(err);
                window.showErrorMessage(`xglass Error: ${err}`);
            });
        }
        ...
        try {
            ...
            console.error(`xglass Error: Please install xprop package to use xglass.`);
            return;
        }
        ...
        function setAlpha(alpha){
            ...
            for(const codeWindowId of codeWindowIds){
               cp.exec(`xprop  -id ${codeWindowId} -f _NET_WM_WINDOW_OPACITY 32c -set _NET_WM_WINDOW_OPACITY $(printf 0x%x $((0xffffffff * ${alpha} / 255)))`, function(error,stdout, stderr){
                    if (error) {
                        console.error(`xglass error: ${error}`);
                        return;
                    }
    ...
    console.log('Congratulations, your extension "xglass VSC" is now active!');
    ...
    context.subscriptions.push(commands.registerCommand('xglass.max', () => {
        setAlpha(1);
    }));

    context.subscriptions.push(commands.registerCommand('xglass.min', () => {
        setAlpha(255);
    }));
